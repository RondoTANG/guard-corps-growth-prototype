...
# 1. Strip ALL inline script tags EXCEPT the tailwind config and the echarts load, and the final script tag
# We'll just remove `<script>` tags that contain `btnOnlyL2` or `btnSelectAll` or `Initial click` inside the body HTML
content = re.sub(r'<script>.*?btnOnlyL2.*?</script>', '', content, flags=re.DOTALL)
content = re.sub(r'<script>\s*// Initial click.*?</script>', '', content, flags=re.DOTALL)

# 2. Re-write the DOMContentLoaded block from scratch to ensure no syntax errors and all features are there
script_start_idx = content.find("document.addEventListener('DOMContentLoaded', function() {")
