...
overlay_html = """
<div id="empty-selection-overlay" class="absolute inset-0 z-50 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center hidden">
    <div class="text-gray-400 text-6xl mb-4"><i class="fas fa-sitemap"></i></div>
    <h3 class="text-xl font-bold text-gray-700 mb-2">未选择党组织</h3>
    <p class="text-sm text-gray-500">请在左侧党组织结构树中，勾选需要分析的党委或党支部</p>
</div>
"""

# 1. Insert overlay right after the <main> element begins, if not exists
if "empty-selection-overlay" not in content:
    target = '<main class="flex-1 flex flex-col overflow-hidden bg-gray-50">'
    idx = content.find(target)
    if idx != -1:
        content = content[:idx+len(target)] + overlay_html + content[idx+len(target):]
    else:
        # Fallback to the flex-1 container for charts
        target2 = '<div class="flex-1 flex flex-col overflow-hidden relative">'
        idx2 = content.find(target2)
        if idx2 != -1:
            content = content[:idx2+len(target2)] + overlay_html + content[idx2+len(target2):]

# 2. Append Javascript to the end of the file (before </body>) if not exists
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnToggleCollapse) {
            ...
            btnToggleCollapse.addEventListener('click', () => {
                ...
                allContainers.forEach(c => {
                    ...
                    if (parentNode && parseInt(parentNode.dataset.level) >= 2) {
                ...
                allIcons.forEach(icon => {
                    ...
                    if (pNode && parseInt(pNode.dataset.level) >= 2) {
                        if (isCollapsed) {
                            icon.classList.remove('fa-chevron-down');
                            icon.classList.add('fa-chevron-right');
                        } else {
            ...
            btnOnlyL2.addEventListener('click', () => {
                ...
                l2Nodes.forEach(node => {
                    ...
                    if(getCheckboxState(box) !== 1) {
                         setCheckboxState(box, 1);
                         
                         const childrenContainer = node.nextElementSibling;
                         if (childrenContainer && childrenContainer.classList.contains('children-container')) {
                             updateChildren(childrenContainer, 1);
                         }
                         updateParents(node);
                    }
                });
        ...
        // Setup Initial State (Default to L2 checked and Tree collapsed)
        setTimeout(() => {
            if (btnOnlyL2) btnOnlyL2.click();
            if (btnToggleCollapse) btnToggleCollapse.click();
        }, 100);
        ...
        if (sidebar && resizer) {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                // For tailwind class w-80 overriding inline style, we might need to remove inline w-80 or use max-width/min-width approach
                sidebar.classList.remove('w-80');
                sidebar.style.width = newWidth + 'px';
                sidebar.style.minWidth = newWidth + 'px'; // ensure it doesn't flex-shrink
...
"""
    ...
    if idx != -1:
        content = content[:idx] + js_content + content[idx:]
    else:
        content += js_content
...
print("Restored JS and overlay successfully!")
