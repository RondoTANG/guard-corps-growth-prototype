...
aside = str(dashboard_soup.find('aside'))
...
# 2. Extract clean tabs
with open('B端_完整后台大盘.html', 'r', encoding='utf-8') as f:
    spa_html = f.read()

spa_soup = BeautifulSoup(spa_html, 'html.parser')
tab_tier = str(spa_soup.find('div', id='tab-tier'))
tab_global = str(spa_soup.find('div', id='tab-global'))
tab_task = spa_soup.find('div', id='tab-task')

# Remove trailing modals from tab_task if they got scooped up
for el in tab_task.find_all('div', id=re.compile('toast|Drawer|modal')):
    el.extract()
tab_task = str(tab_task)
