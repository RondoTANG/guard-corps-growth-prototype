...
# We want to replace from line 471 down to the end of the div containing the textarea.
# Let's find the string:
#                     <p class="text-xs text-gray-400 mt-2">支持JPG、PNG格式图片，最大不超过2M，建议比例2:1</p>
#                         </div>
#                     </div>
# And replace everything after it up to:
#                     <!-- 6. 排序 -->
...
# Regex substitution
html = re.sub(r'支持JPG、PNG格式图片，最大不超过2M，建议比例2:1</p>\s*</div>\s*</div>.*?<!-- 6\. 排序 -->', '支持JPG、PNG格式图片，最大不超过2M，建议比例2:1</p>\n                        </div>\n                    </div>\n\n' + new_editor_html + '\n                    <!-- 6. 排序 -->', html, flags=re.DOTALL)

with open(path, "w", encoding="utf-8") as f:
    f.write(html)

print("Fixed the mangled HTML section.")
