...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var orgOption = {
        tooltip: { 
            ...
            formatter: function(params) {
                let html = '<div class="font-bold mb-1">' + params[0].name + '</div>';
                // Reverse iterate to show L4 at top, L1 at bottom
                for (let i = params.length - 1; i >= 0; i--) {
                    let p = params[i];
                    html += '<div>' + p.marker + ' ' + p.seriesName + ': <span class="font-bold">' + p.value + '%</span></div>';
                }
                return html;
            }
        },
        ...
        series: [
            ...
            { name: '活跃度', type: 'line', yAxisIndex: 1, itemStyle: { color: '#F59E0B' }, symbol: 'circle', symbolSize: 10, lineStyle: { width: 4, shadowColor: 'rgba(245, 158, 11, 0.4)', shadowBlur: 10, shadowOffsetY: 5 }, data: [], z: 10 }
        ]
    };
    ...
    function updateChart(checkedNames) {
        ...
        // Map to correct series index
        orgOption.series[0].data = l1Data; // 新秀
        orgOption.series[1].data = l2Data; // 熟练
        orgOption.series[2].data = l3Data; // 专家
        orgOption.series[3].data = l4Data; // 大师
        orgOption.series[4].data = activeData; // 活跃度
...
print("Chart updated to stacked bar + line.")
